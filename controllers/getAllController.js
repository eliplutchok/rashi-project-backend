...
exports.allEdits = async (req, res) => {
    ...
    const { book, page_number, status, username, currentPage = 1, limit = 20, fetchAll = false, sortField = 'creation_date', sortOrder = 'asc' } = req.query;
    ...
    try {
        ...
        const validSortFields = ['book_name', 'page_number', 'text', 'creation_date', 'status', 'username', 'notes', 'hebrew_text'];
...
exports.allRatings = async (req, res) => {
    logger.info('Query:', req.query);
    const { username, translation_status, rating_status, currentPage = 1, limit = 20, fetchAll = false, sortField = 'creation_date', sortOrder = 'asc' } = req.query;
    const offset = (currentPage - 1) * limit;
    const client = await pool.connect();
    ...
    try {
        ...
        // Add sorting to the query
        const validSortFields = ['book_name', 'page_number', 'text', 'creation_date', 'status', 'username', 'feedback', 'hebrew_text', 'rating'];
        if (validSortFields.includes(sortField)) {
            query += ` ORDER BY ${sortField} ${sortOrder === 'desc' ? 'DESC' : 'ASC'}`;
        }
